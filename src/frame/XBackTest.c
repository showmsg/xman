...
static void read_static(const char *staticfile) {
	...
	while (!feof(fp)) {
		...
		case eDStock:
			...
			break;
		case eTETF:
			memset(&etf, 0, sizeof(XEtfT));
			fread(&etf, sizeof(XEtfT), 1, fp);
			etf.idx = 0;
			if(strlen(etf.fundId))
			{
				XPutVEtf(&etf);
			}
			break;
		case eTETFComp:
			memset(&etfComp, 0, sizeof(XEtfCompT));
			fread(&etfComp, sizeof(XEtfCompT), 1, fp);
			etfComp.idx = 0;
			if(strlen(etfComp.fundId))
			{
				XPutVEtfComp(&etfComp);
				//找到对应的ETF
				pEtf = XFndVEtfByKey(etfComp.fundMkt, etfComp.fundId);
				if(NULL == pEtf)
				{
					slog_error(1, "[%d-%s]ETF加载错误", etfComp.fundMkt, etfComp.fundId);
				}
				if( 0 == pEtf->idxBegin)
				{
					pEtf->idxBegin = etfComp.idx;
					XUpdVEtf(pEtf);
				}
			}
			break;
...
static void SimMkt(XVoid *param) {
	...
	req.tv_sec = 0;         // 设置挂起时间为2秒
	req.tv_nsec = 10;  // 设置挂起时间为10纳秒
	...
	while (!feof(fp)) {
		...
		/** 到批量数据的截止点 */
		iCount++;
		if (mktSimParm->numbers && iCount % mktSimParm->numbers == 0) {
		...
		case eMTickOrder:
			...
			//指定频道
			if (mktSimParm->channel
					&& l2l.order.channel != mktSimParm->channel) {
				break;
			}

			//指定证券代码或者前缀
			if(NULL != mktSimParm->securityId && 0 != strncmp(l2l.order.securityId, mktSimParm->securityId, strlen(mktSimParm->securityId)))
			{
				break;
			}
			l2l.head.market = l2l.order.market;

			//
			if (mktSimParm->breakBiz == l2l.order.bizIndex) {
				memset(sTemp, 0, sizeof(sTemp));
				scanf(sTemp);
				mktSimParm->breakBiz = atol(sTemp);
			}
			l2l.order._recvTime = XGetClockTime();
			if (l2l.order.bsType == -1) {
				l2l.order.bsType = eXBuy;
			}
			if (l2l.order.ordType == -1) {
				l2l.order.ordType = eXOrdAdd;
			}
		...
		case eMTickTrade:
			...
			//指定频道
			if (mktSimParm->channel
					&& l2l.trade.channel != mktSimParm->channel) {
				break;
			}

			//指定证券代码或者前缀
			if(NULL != mktSimParm->securityId && 0 != strncmp(l2l.trade.securityId, mktSimParm->securityId, strlen(mktSimParm->securityId)))
			{
				break;
			}

			l2l.head.market = l2l.trade.market;
			if (mktSimParm->breakBiz == l2l.trade.bizIndex) {
				memset(sTemp, 0, sizeof(sTemp));
				scanf(sTemp);
				mktSimParm->breakBiz = atol(sTemp);
			}
			l2l.trade._recvTime = XGetClockTime();
		...
		case eMSnapshot:
			...
			//指定证券代码或者前缀
			if(NULL != mktSimParm->securityId && 0 != strncmp(l2l.snapshot.securityId, mktSimParm->securityId, strlen(mktSimParm->securityId)))
			{
				break;
			}
...
//	pOrder->frzAmt = pOrder->request.ordPrice * pOrder->request.ordQty;
	...
	slog_debug(0,
			"OnRtnInsert:证券代码[%d-%s],委托序号[%d],买卖方向[%d],委托数量[%d],成交数量[%d],成交金额[%.2f],委托时间[%lld]",
...
static XInt _HandleReject(XOrderT *pOrder) {
	...
	slog_debug(0,
			"OnRtnReject:证券代码[%d-%s],委托序号[%d],买卖方向[%d],委托数量[%d],成交数量[%d],成交金额[%.2f],委托时间[%lld]",
			pOrder->request.market, pOrder->request.securityId,
			pOrder->request.localId, pOrder->request.bsType,
			pOrder->request.ordQty, pOrder->trdQty, pOrder->trdMoney * 0.0001,
			pOrder->_sendLocTime);
...
static XInt _HandleCnfm(XOrderT *pOrder) {
	...
	slog_debug(0,
			"OnRtnOrder:证券代码[%d-%s],委托序号[%d],买卖方向[%d],委托数量[%d],成交数量[%d],成交金额[%.2f]",
			pOrder->request.market, pOrder->request.securityId,
			pOrder->request.localId, pOrder->request.bsType,
			pOrder->request.ordQty, pOrder->trdQty, pOrder->trdMoney * 0.0001);
...
static inline XInt _HandleTrade(XOrderT *pOrder, XQty trdQty, XPrice trdPrice) {
	...
	slog_debug(0,
			"OnRtnTrade: 证券代码[%d-%s] 委托序号[%d],买卖方向[%d],委托数量[%d],成交数量[%d],成交价格[%.3f]",
...
static XInt _HandleHolding(XOrderT *pOrder, XInt isTrade, XQty trdQty,
	...
	if (NULL != pHold) {
		...
		slog_debug(0,
				"OnRtnHold:证券代码[%d-%s],总持仓[%lld], 可用持仓[%lld],成交数量[%d], 冻结持仓[%lld]",
	...
	for (i = 0; i < pMonitor->iTOrder; i++) {
		...
		if (exhOrder.request.isCancel == false) {
			...
			/**
			 ...
			 pOrder->request.securityId);
			 */
			switch (exhOrder.request.bsType) {
			case eXBuy:
				slog_debug(0,
						"[%s-%s]:<<<<<< 证券代码[%d-%s],本地单号[%d],委托数量[%d],委托价格[%d]",
						exhOrder.request.customerId, exhOrder.request.investId,
						exhOrder.request.market, exhOrder.request.securityId,
						exhOrder.request.localId, exhOrder.request.ordQty,
			...
			slog_debug(0,
					"[%s-%s]:XXXXXX 证券代码[%d-%s],本地单号[%d],委托数量[%d],委托价格[%d]",
					exhOrder.request.customerId, exhOrder.request.investId,
					exhOrder.request.market, exhOrder.request.securityId,
					exhOrder.request.localId, exhOrder.request.ordQty,
					exhOrder.request.ordPrice);
			...
			if (NULL != pOrgOrder) {
				...
				if (pOrgOrder->trdQty == 0) {
					...
					_HandleHolding(pOrgOrder, 2,
							pOrgOrder->request.ordQty - pOrgOrder->trdQty, 0);
				}
				//有成交
...
//	gapTime = XGetClockTime() - pOrder->_sendLocTime;
	...
	*/
	...
	if (pOrder->request.ordType == eXOrdLimit) {
		...
		case eXRedeem:
			//直接返回
			pEtf = XFndVEtfByKey(pOrder->request.market, pOrder->request.securityId);
			if(NULL == pEtf)
			{
				break;
			}
			pOrder->trdQty += pOrder->request.ordQty;
			pOrder->ordStatus = eXOrdStatusFilled;
			_HandleCnfm(pOrder);
			_HandleTrade(pOrder, pOrder->trdQty, 0);
			_HandleHolding(pOrder, 1, pOrder->trdQty, 0);
			/**
			for(i = 0; i < pEtfComp; i++)
			{
				_HandleTrade(pOrder, trdQty, trdPx);
				_HandleHolding(pOrder, 1, trdQty, trdPx);
			}
...
static void UpdateHold(XInt market, XChar *securityId, XSumQty qty) {
	...
	for (i = 0; i < pMonitor->iTCust; i++) {
		pCust = XFndVCustomerById(i + 1);
		if (NULL == pCust || pCust->counter != eXCounterOes || eXUserMarket == pCust->type) {
			continue;
		}
...
static void usage() {
	...
	printf("# -l [sleep]回放行情每笔的时间间隔纳秒\n");
	printf("# -p [print] 打印系统信息\n");
	printf("# 举例:\n");
	printf("1. 回测平台启动\n");
	printf("./xbtest -t1 -istatic.bin -qmktstore.bin -v2000 -k200 -c2014 -m1 -s600837\n");
	printf("2. 运行\n");
	printf("./xbtest -t2\n");
	printf("3. 插入持仓\n");
...
int main(int argc, char *argv[]) {
	...
	int opt;
	int option_index = 0;
	const char *option = "ht:m:s:i:q:v:k:pb:c:l:";
	const struct option long_options[] = { { "help", no_argument, NULL, 'h' }, {
			...
			no_argument, NULL, 'p' },{ "sleep", required_argument, NULL,
					'l' },
	...
	while ((opt = getopt_long(argc, argv, option, long_options, &option_index))
		...
		case 'b':
			...
			/** 回放的数量 */
		case 'v':
			mktSimParm.numbers = atoi(optarg);
			break;
			/** 批量回放的时间间隔 */
		case 'k':
	...
	case 15:
		xslog_init(NULL, "xbtest");
		XManShmLoad();
		XBlockPrint(mktSimParm.securityId);
		break;
	//打印板块前8信息
	case 16:
		xslog_init(NULL, "xbtest");
