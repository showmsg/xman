...
static int32
_OnOrder (const MdsL2OrderT *pMdsL2Order)
{
  XL2LT l2lData;
  XTickOrderT order;
  ...
  order.isCancel
      = cancel_from_order (pMdsL2Order->exchId, pMdsL2Order->OrderType);
...
static int32
_OnTrade (const MdsL2TradeT *pMdsL2Trade)
{
  XL2LT l2lData;
  XTickTradeT trade;
...
static int32
_OnL2Snap (const MdsMktDataSnapshotHeadT *pHead,
           const MdsL2StockSnapshotBodyT *pSnapshotBody)
{
  XL2LT l2lData;
  ...
  snapshot.secStatus
      = secstatus_from_mds (snapshot.market, pSnapshotBody->TradingPhaseCode);
  snapshot.mrkStatus
      = mktstatus_from_mds (snapshot.market, pSnapshotBody->TradingPhaseCode);
...
static int32
_OnL1Snap (const MdsMktDataSnapshotHeadT *pHead,
           const MdsStockSnapshotBodyT *pSnapshotBody)
{
  XL2LT l2lData;
  XSnapshotBaseT snapshot;
  XInt j;
...
static int32
_OnIndex (const MdsMktDataSnapshotHeadT *pHead,
           const MdsIndexSnapshotBodyT *pSnapshotBody)
{
  XL2LT l2lData;
  XSnapshotBaseT snapshot;
 ...
 */
static int32
MdsApiSample_HandleMsg (MdsApiSessionInfoT *pSessionInfo, SMsgHeadT *pMsgHead,
...
_MdsQuerySample_OnQueryStockStaticInfo (MdsApiSessionInfoT *pSessionInfo,
                                        SMsgHeadT *pMsgHead, void *pMsgBody,
 ...
 */
static inline int32
_MdsQuerySample_QueryStockStaticInfoList (
    ...
    const MdsQryStockStaticInfoListFilterT *pQryFilter)
{
  ...
  ret = MdsApi_QueryStockStaticInfoList (
      pQryChannel, pSecurityListStr, (char *)NULL, pQryFilter,
      _MdsQuerySample_OnQueryStockStaticInfo, items);
...
Init (const char *configfile)
{
  ...
  if (!MdsApi_InitQryChannel (&cliEnv.qryChannel, configfile, "mds_client",
                              "qryServer"))
    {
...
static int
SubMktData (MdsApiSessionInfoT *pTcpChannel, XInt level, XInt market,
            XChar secType[])
{
  XIdx i = 0, j = 0;
  XBool bFirst = true;
  ...
  if (!level)
    ...
    {
      dataTypes = MDS_SUB_DATA_TYPE_L2_SNAPSHOT | MDS_SUB_DATA_TYPE_L2_ORDER
                  | MDS_SUB_DATA_TYPE_L2_SSE_ORDER
                  | MDS_SUB_DATA_TYPE_L2_TRADE | MDS_SUB_DATA_TYPE_INDEX_SNAPSHOT;
      slog_info (0, "使用L2行情......");
    }
              ...
              if (secType[j] == pStock->secType)
                {
                  if (pStock->secType != eXBond
                      || (pStock->secType == eXBond
                          && (pStock->subSecType == eXSubSecCCF
                              || pStock->subSecType == eXSubSecPRP)))
                    {
                      bSub = true;
                      break;
                    }
          ...
          if (bFirst)
            {
              ...
              bFirst = false;
              /* 根据证券代码列表订阅行情 */
              if (!MdsApi_SubscribeByString (
                      pTcpChannel, buff, (char *)NULL, MDS_EXCH_SSE,
                      MDS_MD_PRODUCT_TYPE_STOCK, MDS_SUB_MODE_SET, dataTypes))
                {
            ...
            {
              /* 根据证券代码列表订阅行情 */
              if (!MdsApi_SubscribeByString (pTcpChannel, buff, (char *)NULL,
                                             MDS_EXCH_SSE,
                                             MDS_MD_PRODUCT_TYPE_STOCK,
  ...
  /* 根据证券代码列表订阅行情 */
  if (icount % 90 != 0
      && !MdsApi_SubscribeByString (pTcpChannel, buff, (char *)NULL,
  ...
  /* 订阅指数行情 */
  if (!MdsApi_SubscribeByString (
  		pTcpChannel, (char *)NULL, (char *)NULL, MDS_EXCH_SSE,
		MDS_MD_PRODUCT_TYPE_INDEX, MDS_SUB_MODE_APPEND, dataTypes))
    {
  	slog_error (0, "根据证券代码列表订阅行情失败!");
  	return (-1);
    }
  ...
  /* 订阅指数行情 */
  if (!MdsApi_SubscribeByString (
  		pTcpChannel, (char *)NULL, (char *)NULL, MDS_EXCH_SZSE,
		MDS_MD_PRODUCT_TYPE_INDEX, MDS_SUB_MODE_APPEND, dataTypes))
    {
  	slog_error (0, "根据证券代码列表订阅行情失败!");
  	return (-1);
...
MdsApiSample_ReSubMkt (SessionWithLevelT *sessionWithLevel)
{
  ...
  while (!*pThreadTerminatedFlag)
    {
      if (XIsNullCache (XSHMKEYCONECT (mktSubscribe))
          || (security = XPopCache (XSHMKEYCONECT (mktSubscribe), readId))
                 == NULL)
        {
          continue;
        }
      ...
      if (security->type == exReset)
        {
          if (!MdsApi_SubscribeByString (
                  pTcpChannel, security->securityId, (char *)NULL, market,
                  MDS_MD_PRODUCT_TYPE_STOCK, MDS_SUB_MODE_SET, dataTypes))
            {
              slog_error (0, "根据证券代码列表[%s]订阅行情失败!",
                          security->securityId);
              //					return FALSE;
            }
          else
            {
              slog_info (3, "实时订阅行情，重置行情,重新订阅[%s]",
                         security->securityId);
            }
        }
      ...
      else if (security->type == exDelete)
        {
          if (!MdsApi_SubscribeByString (
                  pTcpChannel, security->securityId, (char *)NULL, market,
                  MDS_MD_PRODUCT_TYPE_STOCK, MDS_SUB_MODE_DELETE, dataTypes))
            {
              slog_error (0, "根据证券代码列表[%s]订阅行情失败!",
                          security->securityId);
            }
      ...
      else
        {
          ...
          slog_info (3, "实时订阅行情[%d-%s]", security->market,
                     security->securityId);
          if (!MdsApi_SubscribeByString (
                  pTcpChannel, security->securityId, (char *)NULL, market,
                  MDS_MD_PRODUCT_TYPE_STOCK, MDS_SUB_MODE_APPEND, dataTypes))
            {
              slog_error (0, "根据证券代码列表[%s]订阅行情失败!",
                          security->securityId);
              //					return FALSE;
...
XInt
oesmkt (XChar *customer, XBindParamT *pBind)
{
  /* 尝试等待行情消息到达的超时时间 (毫秒) */
...
ON_RECONNECT:
  ...
  if (!MdsApi_InitAll (&cliEnv, XMDS_CONFIG_FILE,
                       ...
                       MDSAPI_CFG_DEFAULT_SECTION, "tcpServer", (char *)NULL,
  ...
  slog_warn (0, "!!!!!!   开始接收行情数据[%lld]......   !!!!!!",
             XGetComMSec ());

  if (pBind->resub)
    {
      slog_info (3,
                 "开启盘中实时订阅功能(开盘后订阅的证券重构数据会丢失)......");

      pthread_t subThreadId;

      sessionWithLevel.level = pBind->level;
      sessionWithLevel.session = cliEnv.tcpChannel;
      iret = pthread_create (&subThreadId, NULL,
                             (void *(*)(void *))MdsApiSample_ReSubMkt,
                             &sessionWithLevel);
      if (unlikely (iret != 0))
        {
  ...
  else
    {
      ...
      SubMktData (&cliEnv.tcpChannel, pBind->level, pBind->market,
                  pBind->secType);
    }
  for (;;)
    {
      ...
      iret = MdsApi_WaitOnMsg (&cliEnv.tcpChannel, THE_TIMEOUT_MS,
                               MdsApiSample_HandleMsg, (void *)(pBind));
      if (iret < 0)
        {
