...
int read_basket(const char *trade_file, XSettingT trade_list[])
{
	...
	while ((!XCsvReadLine(&handle)))
	{
		...
		if (col)
		{
			...
			trade_list[rowcount].stopProfit = atoi(col);
		}
		col = handle.GetFieldByCol(&handle, 4);
		if (col)
		{
			trade_list[rowcount].ordPx = atoi(col);
			trade_list[rowcount].stopLoss = atoi(col);
		}
...
int read_plot(XIniT *strategyhndl, const char *section, XPlotT *request)
{
	...
	if (!strcmp(pSectionConf, "Basket"))
	{
		request->plotType = eXMidRobPlot;
		slog_info(3, "旁中抢板策略......");
	}
	else if(!strcmp(pSectionConf, "BondRob"))
	{
		request->plotType = eXAuctionRobPlot;
		slog_info(3, "集合竞价抢单策略......");
	}
	else if(!strcmp(pSectionConf, "Bond"))
	{
		request->plotType = eXBondT0Plot;
		slog_info(3, "可转债策略......");
	}
...
XInt XLoadHis(XInt market, XChar *security, XPlotId plotid)
{
	...
	for (i = 0; i < pMonitor->iTOrder; i++)
	{
		...
		if (!pFndOrder->_locFlag && plotid == pFndOrder->request.plotid)
		{
			...
			if (NULL != pStrategy && pStrategy->setting.market == pFndOrder->request.market && 0 == strcmp(pFndOrder->request.securityId, pStrategy->setting.securityId))
			{
...
int init_strategy(XPlotT *request)
{
	...
	for (i = 0; i < iBuyholdCnt; i++)
	{
		...
		pStrategy = XFndVStrategyByFrontId(request->customerId, 0,
											request->frontId, basketList[i].securityId, basketList[i].bsType);
		if (NULL == pStrategy)
		{
			// 添加策略
			...
			if(basketList[i].qtyType)
			{
				...
				{
					strategy.setting.ordQty = basketList[i].money * 10000 / (pStock->upperPrice * pStock->lmtBuyMinQty) *  pStock->lmtBuyMinQty;
				...
				{
					slog_warn(0, "[%d-%s]无涨停价", basketList[i].market, basketList[i].securityId);
				}
			}
			else
			{
			...
			{
				pHold = XFndVHoldByKey(strategy.plot.customerId, strategy.investId, strategy.setting.market, strategy.setting.securityId);

				if(strategy.setting.ordQty)
				{
					if(NULL != pHold && strategy.setting.ordQty != pHold->sellAvlHld)
					{
						slog_warn(0, "[%s-%lld] 证券代码[%d-%s],卖出时委托数量[%d]不等于可卖数量[%lld]", strategy.plot.customerId, strategy.plotid,
						strategy.setting.market, strategy.setting.securityId, strategy.setting.ordQty, pHold->sellAvlHld);
					}
				}
				else
				{
					if(NULL != pHold)
					{
						...
						slog_warn(0, "[%s-%lld] 证券代码[%d-%s],未设置卖出数量,以持仓可卖数量进行卖出[%lld]", strategy.plot.customerId, strategy.plotid,
												strategy.setting.market, strategy.setting.securityId,  pHold->sellAvlHld);
					}
				}

			}

			if (basketList[i].isnontrade || request->isForbid || strategy.setting.ordQty <= 0)
			{
			...
			XLoadHis(strategy.setting.market, strategy.setting.securityId, strategy.plotid);

			PushStrategy(&strategy);
			slog_info(3,
					  "添加策略[%s-%lld],证券代码[%d-%s],信号价格[%d],买卖价格[%d],委托数量[%d],买一撤单率[%d],卖一量[%lld],买一委托金额[%lld], 止盈[%d],止损[%d],状态[%d],模式[%d]",
					  strategy.plot.customerId, strategy.plotid, strategy.setting.market, strategy.setting.securityId, 
					  strategy.setting.conPx, strategy.setting.ordPx, strategy.setting.ordQty, strategy.setting.cdl,
					  strategy.setting.askQty, strategy.setting.buyMoney,
					  strategy.setting.stopProfit, strategy.setting.stopLoss, 
		...
		else
		{
			...
			if(basketList[i].qtyType)
			{
				if(pStock->upperPrice && pStock->lmtBuyMinQty)
				{
					pStrategy->setting.ordQty = basketList[i].money * 10000 / (pStock->upperPrice * pStock->lmtBuyMinQty) *  pStock->lmtBuyMinQty;
			...
			if(pStrategy->setting.bsType == eXSell)
			{
				...
				if(pStrategy->setting.ordQty)
				{
					if(NULL != pHold && pStrategy->setting.ordQty != pHold->sellAvlHld)
					{
						slog_warn(0, "[%s-%lld] 证券代码[%d-%s],卖出时委托数量[%d]不等于可卖数量[%lld]", pStrategy->plot.customerId, pStrategy->plotid,
						pStrategy->setting.market, pStrategy->setting.securityId, pStrategy->setting.ordQty, pHold->sellAvlHld);
					}
				}
				else
				{
					if(NULL != pHold)
					{
						...
						slog_warn(0, "[%s-%lld] 证券代码[%d-%s],未设置卖出数量,以可卖数量进行卖出[%lld]", pStrategy->plot.customerId, pStrategy->plotid,
						pStrategy->setting.market, pStrategy->setting.securityId, pHold->sellAvlHld);
					}
				}
			...
			pStrategy->setting.stopLoss = basketList[i].stopLoss;
			pStrategy->setting.stopProfit = basketList[i].stopProfit;
			...
			if (basketList[i].isnontrade || request->isForbid || pStrategy->setting.ordQty <= 0)
			{
			...
			slog_info(3,
					  "修改策略[%s-%lld],证券代码[%d-%s],信号价格[%d],买卖价格[%d],委托数量[%d],买一撤单率[%d],卖一量[%lld],买一委托金额[%lld], 止盈[%d],止损[%d],状态[%d]，模式[%d]",
					  pStrategy->plot.customerId, pStrategy->plotid, pStrategy->setting.market, pStrategy->setting.securityId, 
					  pStrategy->setting.conPx, pStrategy->setting.ordPx, pStrategy->setting.ordQty, pStrategy->setting.cdl,
					  pStrategy->setting.askQty, pStrategy->setting.buyMoney,
					  pStrategy->setting.stopProfit, pStrategy->setting.stopLoss, 
...
int main(int argc, char *argv[])
{
	...
	const struct option long_options[] = {{"help", no_argument, NULL, 'h'}, {"type", required_argument, NULL, 't'}, {"market", required_argument, NULL, 'm'}, {"security", required_argument, NULL, 's'}, {"bstype", required_argument, NULL, 'b'}, {"plotid", required_argument, NULL, 'p'}, {"customer", required_argument, NULL, 'c'}, {"localid", required_argument, NULL, 'l'}, {"run", required_argument, NULL, 'r'}, {"localids", required_argument, NULL, 'u'}, {"cdl", required_argument, NULL, 'd'}, {NULL, 0, NULL, 0}};
	...
	case 3:
		...
		for (i = 0; i < pMonitor->iTPlot; i++)
		{
			...
			if (NULL == pStrategy || 0 != strncmp(pStrategy->setting.securityId, security, 6))
			{
				continue;
			}
			/** 启动 */
			if (1 == run)
			{
				...
				slog_warn(0, ">>>>>> [%s-%s],策略编号[%lld],启动",
						  pStrategy->plot.customerId, pStrategy->setting.securityId,
						  pStrategy->plotid);
			}
			/** 停止 */
			else
	...
	case 5:
		...
		for (j = 0; j < icnt; j++)
		{
			...
			for (i = 0; i < pMonitor->iTPlot; i++)
			{
				...
				if (NULL == pStrategy || pStrategy->plot.plotType != eXMidRobPlot
				|| 0 != strncmp(pStrategy->setting.securityId, security, 6) 
				|| 0 != strncmp(pStrategy->plot.customerId, customer, strlen(customer))
				...
				slog_info(0, "[%lld-%d-%s]localid:%d, cdl:%d", pStrategy->plotid, pStrategy->setting.market, pStrategy->setting.securityId, pStrategy->plot.frontId, pStrategy->setting.cdl);
				break;
			}
		}
		break;
